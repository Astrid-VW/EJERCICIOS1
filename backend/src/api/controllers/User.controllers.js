  ...
  try {
    ...
    if (!userExist) {
      ...
      if (req.file) {
        ...
        newUser.image = "https://res.cloudinary.com/dewx8rqgv/image/upload/v1706799752/default_user_hmvhhd.png";
      }
      ...
      try {
          ...
          transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
              ...
              return res.status(404).json({
                user: userSave,
                confirmationCode: `error, resend code ${error.message}`,
              });
            }
        ...
        console.log("Error nuevo", error.message)
...
const registerMedio = async (req, res, next) => {
  ...
  try {
    ...
    if (!userExists) {
      ...
      if (confirmationCode === userExists.confirmationCode) {
        console.log(confirmationCode, userExists.confirmationCode)
        try {
...
//? -----------------------------------------------------------------------------
//! ------------------ CAMBIO DE CONTRASEÑA DENTRO DEL LOGIN -----------------
//? -----------------------------------------------------------------------------
